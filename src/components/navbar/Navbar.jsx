...
const Navbar = ({ children }) => {
    ...
    return (
        <>
            <div className={"fixed z-50 top-0 left-0 w-full transition-transform duration-300 ease-linear backdrop-blur-lg bg-white dark:bg-slate-950 bg-opacity-10 dark:bg-opacity-10 " + (show ? " translate-y-0 " : " -translate-y-32 ") + (shadow ? " shadow-md " : " ")} >
                <nav className="max max-w-screen-xl w-full mx-auto px-4 md:px-10 h-16 flex justify-between items-center" >
                    <Link to={"/"} className="h-16 overflow-hidden flex items-center" >Redefine Status</Link>
